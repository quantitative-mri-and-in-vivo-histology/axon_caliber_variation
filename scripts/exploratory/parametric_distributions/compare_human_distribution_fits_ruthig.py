"""
Compare distribution fits: Human CC LM vs Human CC EM (Ruthig et al.) data.

Creates a 2x2 figure showing:
- (a) Human LM: pooled histogram with fitted PDFs
- (b) Human EM: pooled histogram with fitted PDFs
- (c) Human LM: AIC comparison (summed across all ROIs)
- (d) Human EM: AIC comparison (summed across all ROIs)

This script focuses purely on AIC-based model comparison. Separate scripts
handle r_eff and r_arith evaluation.

EM data source:
    Ruthig AT et al. (2025). Electron microscopy axon morphometry data of
    the human corpus callosum [Data set]. Zenodo.
    https://doi.org/10.5281/zenodo.15601202

Usage:
    python scripts/exploratory/parametric_distributions/compare_human_distribution_fits_ruthig.py \\
        --human-data data/raw/human/lm \\
        --em-data data/raw/human/em_ruthig/CC_anonymized.csv \\
        --output fig/exploratory/compare_human_distribution_fits_ruthig.svg
"""
...
from axonometry.distribution_fitting import FitResult, fit_distribution_mle, compute_distribution_radii
...
# =============================================================================
# Data Loading - Human EM (Ruthig et al.)
# =============================================================================
...
def load_em_data(
    ...
    """Load EM human CC data from Ruthig et al."""
...
# =============================================================================
# Distribution Fitting (imported from axonometry.distribution_fitting)
# =============================================================================
    ...
    for sample_idx in range(n_samples):
        ...
        for dist_idx, (dist_name, dist) in enumerate(CANDIDATE_DISTRIBUTIONS):
            ...
            if result is not None:
                ...
                r_arith, r_eff = compute_distribution_radii(dist, result.params, r_max=r_max_eval)
...
def create_combined_figure(
    ...
    em_pooled: HistogramData,
    em_metrics: AggregatedMetrics,
    human_per_sample: PerSampleHistogramData,
    em_per_sample: PerSampleHistogramData,
    output_file: Path,
    top_n: int = 5
) -> None:
    ...
    HUMAN_COLOR = settings.colors['binary_a']  # Sand/tan for LM
    EM_COLOR = settings.colors['binary_b']  # Dusty teal for EM
    ...
    ax_a = fig.add_subplot(gs[0, 0:2])  # EM PDF
    ...
    class SplitColorHandler(HandlerBase):
        ...
        def create_artists(self, legend, orig_handle, xdescent, ydescent,
            ...
            # Create two rectangles side by side (EM first, then LM)
            from matplotlib.patches import Rectangle
            half_width = width / 2
            left_rect = Rectangle(
                (xdescent, ydescent), half_width, height,
                facecolor=EM_COLOR, edgecolor='white', linewidth=0.5,
    ...
    # (c) Win rate
    _plot_win_rate(
        ax_c, human_metrics, em_metrics,
        human_color=HUMAN_COLOR, em_color=EM_COLOR
    )

    # (d) Wasserstein distance with inter-ROI reference
    _plot_wasserstein_both_species(
        ax_d, human_metrics, em_metrics,
        human_color=HUMAN_COLOR, em_color=EM_COLOR,
        human_inter_roi=human_inter_roi_w, em_inter_roi=em_inter_roi_w
    )

    # (e) r_arith error - both species
    _plot_radius_bias_both_species(
        ax_e, human_metrics, em_metrics,
        radius_type='r_arith',
        human_color=HUMAN_COLOR, em_color=EM_COLOR
    )
...
def _plot_win_rate(
    ...
    em_win = np.array([em_metrics.win_rate.get(n, 0) for n in names]) * 100

    y_spacing = 1.3  # Spacing between distributions
    y_pos = np.arange(len(names)) * y_spacing

    # Plot dots (EM first so LM is on top)
    ax.scatter(em_win, y_pos, color=em_color, s=120, marker='s',
               label='EM', zorder=3, edgecolor='white', linewidth=0.5)
    ax.scatter(human_win, y_pos, color=human_color, s=120, marker='d',
               label='Human', zorder=4, edgecolor='white', linewidth=0.5)

    # Connect dots with lines
    for i in range(len(names)):
        ax.plot([human_win[i], em_win[i]], [y_pos[i], y_pos[i]],
...
def _plot_wasserstein_both_species(
    ...
    for dist_name in names:
        ...
        # EM Wasserstein values
        em_dist_idx = em_metrics.dist_name_to_idx.get(dist_name, -1)
        if em_dist_idx >= 0:
            em_w = em_metrics.all_wasserstein[em_dist_idx]
            em_w_valid = em_w[~np.isnan(em_w)]
            box_data_em.append(em_w_valid)
        else:
            box_data_em.append(np.array([]))
    ...
    # Plot boxplots for EM (offset up)
    em_positions = [y_pos[i] + box_width/2 for i in range(len(box_data_em))]
    bp_em = ax.boxplot([d if len(d) > 0 else [np.nan] for d in box_data_em],
                        positions=em_positions, vert=False, widths=box_width * 0.8,
                        patch_artist=True, showfliers=False,
                        medianprops=dict(color=em_color, linewidth=2),
                        whiskerprops=dict(color=em_color, linewidth=1.5),
                        capprops=dict(color=em_color, linewidth=1.5))
    for patch in bp_em['boxes']:
        patch.set_facecolor(em_color)
        patch.set_alpha(0.5)
        patch.set_edgecolor(em_color)
    ...
    if human_inter_roi is not None:
        ax.axvline(human_inter_roi, color=human_color, linestyle='--', linewidth=2,
                   alpha=0.8, zorder=1)
    if em_inter_roi is not None:
        ax.axvline(em_inter_roi, color=em_color, linestyle='--', linewidth=2,
                   alpha=0.8, zorder=1)

    # Add legend manually (EM, LM on first row; Anat. Var. on second row)
    ...
    # To get Row1: [EM, LM], Row2: [Anat. Var.], order must be: [EM, Anat. Var., LM]
    em_handle = Patch(facecolor=em_color, label='EM')
    human_handle = Patch(facecolor=human_color, label='Human')

    if human_inter_roi is not None or em_inter_roi is not None:
        # Placeholder handle for Anat. Var. (handler will draw the actual symbol)
        anat_var_handle = Line2D([0], [0], color='none')
        # Single row: EM, LM, Anat. Var.
        legend_elements = [em_handle, human_handle, anat_var_handle]
        labels = ['EM', 'Human', 'Anat. Var.']
        handler_map = {
            em_handle: SolidPatchHandler(em_color),
            human_handle: SolidPatchHandler(human_color),
            anat_var_handle: DualDashedLineHandler(em_color, human_color)
        }
        ncol = 3
    else:
        legend_elements = [em_handle, human_handle]
        labels = ['EM', 'Human']
        handler_map = {
            em_handle: SolidPatchHandler(em_color),
            human_handle: SolidPatchHandler(human_color)
...
def _plot_radius_bias_both_species(
    ...
    Args:
        ...
        em_metrics: Human EM aggregated metrics
        radius_type: 'r_arith' or 'r_eff'
        human_color: Color for human data points
        em_color: Color for EM data points
    """
    # Use human distribution order (sorted by AIC)
    names = human_metrics.distribution_names
    display_names = [get_display_name(n, multiline=True) for n in names]
    ...
    for dist_name in names:
        ...
        em_dist_idx = em_metrics.dist_name_to_idx.get(dist_name, -1)
        if em_dist_idx >= 0:
            em_fitted = em_all[em_dist_idx]
            em_bias = (em_fitted - em_emp_per_sample) / em_emp_per_sample * 100
            em_bias_valid = em_bias[~np.isnan(em_bias)]
            box_data_em.append(em_bias_valid)
        else:
            box_data_em.append(np.array([]))
    ...
    for i, (h_data, r_data) in enumerate(zip(box_data_human, box_data_em)):
        # Human
        if len(h_data) > 0:
        ...
        if len(r_data) > 0:
            ...
            if r_median > x_lim:
                ax.annotate(f'{r_median:.0f}%', xy=(x_lim, y_pos[i] + box_width/2),
                            xytext=(x_lim - 8, y_pos[i] + box_width/2),
                            fontsize=annot_fontsize, fontweight='bold',
                            color=em_color, ha='right', va='center',
                            arrowprops=dict(arrowstyle='->', color=em_color, lw=1.5))
            elif r_median < -x_lim:
                ax.annotate(f'{r_median:.0f}%', xy=(-x_lim, y_pos[i] + box_width/2),
                            xytext=(-x_lim + 8, y_pos[i] + box_width/2),
                            fontsize=annot_fontsize, fontweight='bold',
                            color=em_color, ha='left', va='center',
                            arrowprops=dict(arrowstyle='->', color=em_color, lw=1.5))

    # Add legend with boxplot-style handles (EM first)
    from matplotlib.patches import Patch
    from matplotlib.lines import Line2D
    from matplotlib.legend_handler import HandlerBase
    ...
    em_patch = Patch(facecolor=em_color, label='EM', alpha=1.0)
    human_patch = Patch(facecolor=human_color, label='Human', alpha=1.0)

    ax.legend(handles=[em_patch, human_patch], loc='upper right',
              fontsize=settings.fonts['legend_size'], ncol=2,
              frameon=True, edgecolor='gray', facecolor='white', framealpha=1.0,
              handletextpad=0.3, borderpad=0.3,
              columnspacing=0.8, handlelength=1.8,
              handler_map={em_patch: BoxplotHandler(em_color, em_color),
...
def main():
    parser = argparse.ArgumentParser(
        description='Compare distribution fits: Human CC LM vs EM (Ruthig et al.)',
    ...
    parser.add_argument('--em-data', type=Path, default=Path('data/raw/human/em_ruthig/CC_anonymized.csv'),
                        help='Path to Ruthig EM CSV file')
    parser.add_argument('--output', type=Path, default=Path('fig/exploratory/compare_human_distribution_fits_ruthig.svg'),
    ...
    # Load EM data (Ruthig et al.)
    logger.info("=" * 60)
    logger.info("Loading Human EM data (Ruthig et al.)...")
    em_pooled, em_per_sample = load_em_data(args.em_data, args.bin_width, args.r_max)
    ...
    logger.info("=" * 60)
    logger.info("Fitting Human EM distributions...")
    em_metrics = fit_all_samples(em_per_sample, em_pooled)
    ...
    logger.info("Human EM - Top 5 by summed AIC:")
    ...
    create_combined_figure(
        ...
        em_pooled, em_metrics,
        human_per_sample, em_per_sample,
    ...
    output_data = {
        ...
        'human_em': {
            'n_rois': em_pooled.n_samples,
            'total_count': em_pooled.total_count,
            'distributions': [
                {'name': name, 'summed_aic': float(em_metrics.summed_aic[i])}
                for i, name in enumerate(em_metrics.distribution_names)
            ]
        }
    }
