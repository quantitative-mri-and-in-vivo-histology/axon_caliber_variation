...
from analyze_effective_radius import analyze_population
...
def batch_analyze(input_dir: Path,
                  ...
                  max_ellipse_ratio: float = 0.0) -> Dict[str, Dict[str, Tuple[float, np.ndarray]]]:
    """
    Analyze effective radius for all *_spatial.zarr files in input directory.

    Args:
        input_dir: Directory containing *_spatial.zarr files
        output_dir: Directory for output plots
        voxel_size_um: Voxel size in micrometers (default: 0.2 for 4x downsampled)
    ...
    logger.info(f"Found {len(zarr_files)} spatial Zarr files to analyze")
    ...
    for i, zarr_file in enumerate(zarr_files, 1):
        ...
        logger.info(f"\n{'='*80}")
        logger.info(f"Analyzing {i}/{len(zarr_files)}: {zarr_file.name}")
        ...
        try:
            ...
            logger.info(f"Successfully analyzed {sample_name}")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Batch analyze effective radius from spatial subvolumes (CC/CG)'
    )
    ...
    parser.add_argument('--voxel-size', type=float, default=0.2,
                        help='Voxel size in micrometers (default: 0.2 for 4x downsampled)')
