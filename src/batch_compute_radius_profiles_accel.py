#!/usr/bin/env python3
"""
Batch computation of axon radius profiles from labeled volumes (ACCELERATED).

Uses optimized skeleton extraction with Numba JIT compilation (~2.8x faster).
...
from compute_axon_radius_profiles_accel import compute_radius_profiles, parse_voxel_size_arg
...
def batch_compute_profiles(input_dir: Path,
    ...
    logger.info(f"Using path_method='{path_method}' (accelerated skeleton extraction)")
    ...
    for i, mat_file in enumerate(mat_files, 1):
        ...
        output_file = output_dir / f'{sample_name}_radius_profiles_accel.npz'
    ...
    # Summary
    logger.info(f"\n{'='*80}")
    logger.info("BATCH PROCESSING COMPLETE (ACCELERATED)")
    logger.info(f"{'='*80}")
    ...
    if all_results:
        ...
        summary_file = output_dir / 'radius_profiles_accel_summary.json'
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Batch compute axon radius profiles from labeled volumes (ACCELERATED with Numba)'
    )
